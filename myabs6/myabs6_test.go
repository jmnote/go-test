...
func TestAbs_style5(t *testing.T) {
	testCases := []struct {
		name string
		num  int
		want int
	}{
		{"negative", -1, 9999},    // negative
		{"non-negative", 0, 9999}, // non-negative
		{"non-negative", 1, 9999}, // non-negative#01
	}
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			got := Abs(tc.num)
...
func TestAbs_style6(t *testing.T) {
	testCases := []struct {
		num  int
		want int
	}{
